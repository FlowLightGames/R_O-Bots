...
func _on_send_pressed()->void:
	...
	text=text.replace(" ","")
	if !text.is_empty():
		var msg:PackedByteArray=PackageConstructor.lobby_chat_message(SteamLobby.player_number,line_input.text)
...
func _on_line_edit_text_submitted(new_text:String)->void:
	...
	text=text.replace(" ","")
	if !text.is_empty():
		var msg:PackedByteArray=PackageConstructor.lobby_chat_message(SteamLobby.player_number,new_text)
		SteamLobby.send_p2p_packet(0,Steam.P2P_SEND_RELIABLE,msg)
